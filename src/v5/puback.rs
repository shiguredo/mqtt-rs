...
impl PubAck {
    ...
    /// PUBACK パケットを `buf` にエンコードし、書き込んだバイト数を返す。
    ...
    pub fn decode(buf: &[u8]) -> Result<(Self, usize), DecodeError> {
        let (packet_id, reason_code, properties, consumed) = ack_helper::decode_reason_code_packet(
            ...
            Self::PACKET_TYPE,
            Self::FLAGS,
            PubAckReasonCode::Success as u8,
        )?;
        properties.validate_for_puback()?;
        let reason_code =
            PubAckReasonCode::from_u8(reason_code).ok_or(DecodeError::MalformedPacket)?;
        Ok((
            Self {
                packet_id,
                reason_code,
    ...
    fn validate(&self) -> Result<(), EncodeError> {
        ...
        if self.reason_code == PubAckReasonCode::NoMatchingSubscribers {
            return Err(EncodeError::InvalidField {
                reason: EncodeInvalidField::InvalidReasonCode,
            });
        }
        if self.properties.validate_for_puback().is_err() {
            return Err(EncodeError::InvalidField {
