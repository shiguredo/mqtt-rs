...
/// MQTT v5.0 Unsubscribe 理由コード。
///
/// MQTT v5.0 §3.11.3 を参照。
...
impl UnsubAck {
    ...
    pub fn decode(buf: &[u8]) -> Result<(Self, usize), DecodeError> {
        ...
        while offset < end {
            let code =
                UnsubAckReasonCode::from_u8(buf[offset]).ok_or(DecodeError::MalformedPacket)?;
    ...
    fn validate(&self) -> Result<(), EncodeError> {
        ...
        if self.properties.validate_for_unsuback().is_err() {
            return Err(EncodeError::InvalidField {
