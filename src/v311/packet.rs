//! クライアント目線の送信・受信に分離した MQTT v3.1.1 パケット列挙型。
//!
//! MQTT v3.1.1 §2.2 を参照。
//!
...
use super::pingreq::PingReq;
use super::pingresp::PingResp;
...
impl OutgoingPacket {
    ...
    pub fn encoded_len(&self) -> usize {
        match self {
            ...
            OutgoingPacket::PingReq(p) => p.encoded_len(),
...
/// クライアントから見て受信する（Server → Client）MQTT v3.1.1 制御パケット。
///
/// CONNECT / SUBSCRIBE / UNSUBSCRIBE / PINGREQ / DISCONNECT の
/// Client → Server 専用種別はバリアントとして存在せず、
/// コンパイル時に受信不可能であることが保証される。
...
impl IncomingPacket {
    ...
    pub(crate) fn decode(buf: &[u8]) -> Result<(Self, usize), DecodeError> {
        ...
        // MQTT v3.1.1 §2.2.2 [MQTT-2.2.2-1] に基づき、各パケット種別の予約フラグは指定値である必要がある。
        match buf[0] & 0xF0 {
            0x20 => {
            ...
            // MQTT v3.1.1 §2.2.1 Table 2.1 を参照。MQTT v3.1.1 では DISCONNECT も
            // Client → Server 専用である（MQTT v3.1.1 §3.14）。
            packet_type @ (0x10 | 0x80 | 0xA0 | 0xC0 | 0xE0) => {
                Err(DecodeError::UnexpectedPacket { packet_type })
            }
            // 0x00 と 0xF0 は Reserved 種別のため、方向拒否ではなく種別不正として拒否する。
            // MQTT v3.1.1 §2.2.1 Table 2.1 を参照。
            0x00 | 0xF0 => Err(DecodeError::InvalidPacketType),
...
pub mod codec {
    ...
    /// 方向を跨いだ全種別を扱う低水準の MQTT v3.1.1 制御パケット。
    ///
    ...
    impl Packet {
        ...
        /// `buf` から MQTT v3.1.1 制御パケットをデコードする。
        ///
        /// デコードしたパケットと消費したバイト数を返す。
        ///
