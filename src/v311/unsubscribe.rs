//! MQTT v3.1.1 UNSUBSCRIBE パケット。
//!
//! MQTT v3.1.1 §3.10 を参照。
...
impl Unsubscribe {
    ...
    pub fn encoded_len(&self) -> usize {
        ...
        for topic_filter in &self.topic_filters {
            len = len.saturating_add(2).saturating_add(topic_filter.len());
        }
        len
    }
    ...
    pub fn encode(&self, buf: &mut [u8]) -> Result<usize, EncodeError> {
        ...
        // MQTT v3.1.1 §3.10.3 [MQTT-3.10.3-2]: Topic Filter は 1 つ以上必要。
        ...
        for filter in &self.topic_filters {
            if !crate::topic_filter::is_valid_topic_filter(filter) {
                return Err(EncodeError::InvalidField {
        ...
        for topic_filter in &self.topic_filters {
            offset += Utf8String::encode_str(topic_filter, &mut buf[offset..])?;
        }
    ...
    pub fn decode(buf: &[u8]) -> Result<(Self, usize), DecodeError> {
        ...
        while offset < end {
            ...
            topic_filters.push(topic_filter.0);
        }
