//! Sans-I/O な `shiguredo_mqtt` を tokio の TCP / TLS 上で動かす MQTT v5.0 クライアント。
//!
...
/// ブローカーとの送受信に使うトランスポート。
///
/// 平文 TCP と mqtts (TCP + TLS) を同一クライアント実装で扱う。
enum Transport {
    /// 平文 TCP。
    Tcp(TcpStream),
    /// TLS ラップ済みのストリーム。
...
impl MqttClient {
    ...
    pub async fn connect_tcp(host: &str, port: u16) -> Result<Self, Error> {
        ...
        info!(%addr, "connecting TCP");
        let stream = TcpStream::connect(&addr).await?;
        // 小さな制御パケットのバースト遅延を避ける。
    ...
    pub async fn connect_tls(
        ...
        let addr = format!("{host}:{port}");
        info!(%addr, %server_name, "connecting mqtts (TCP + TLS)");

        let mut roots = RootCertStore::empty();
        let ca = CertificateDer::from_pem_slice(ca_pem.as_bytes())
            .map_err(|e| Error::Tls(format!("CA certificate PEM: {e}")))?;
        roots
            .add(ca)
            .map_err(|e| Error::Tls(format!("RootCertStore add: {e}")))?;
        ...
        Ok(Self {
            stream: Transport::Tls(Box::new(tls)),
            session: Session::new_v5(String::new(), true)?,
            decoder: Decoder::new(MqttVersion::V5),
            clock: Clock::new(),
        })
    ...
    ///
    /// MQTT DISCONNECT は送らず、ストリームごと破棄する。
