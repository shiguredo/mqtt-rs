//! s2n-quic ベースの MQTT over QUIC v5.0 クライアント example。
//!
//! `shiguredo_mqtt` は Sans-I/O のため、QUIC 接続とイベントループは
//! この example 側で tokio / s2n-quic を使って実装する。
//!
//! 使い方:
//! ```text
//! # メッセージを公開する（CA 証明書と SNI が必要）
...
async fn run_publish(opts: PublishOptions) -> Result<(), Error> {
    let ca_pem = std::fs::read_to_string(&opts.common.ca_file)?;
    let mut client = MqttClient::connect_quic(
        &opts.common.host,
        opts.common.port,
        &ca_pem,
        &opts.common.server_name,
    )
    .await?;
    client
...
fn parse_common_options(args: &mut noargs::RawArgs) -> noargs::Result<CommonOptions> {
    ...
    let host: String = noargs::opt("host")
        ...
        .take(args)
        .then(|o| Ok::<_, std::convert::Infallible>(o.value().to_string()))?;
    // EMQX の MQTT over QUIC 既定ポート。
    let port: u16 = noargs::opt("port")
        .short('p')
        .ty("PORT")
        .doc("Broker QUIC (UDP) port number")
        .default("14567")
        .take(args)
        .then(|o| o.value().parse())?;
    let ca_file: PathBuf = noargs::opt("ca-file")
        .ty("PATH")
        .doc("Path to CA certificate PEM used as TLS trust anchor")
        .example("/path/to/ca.pem")
        .take(args)
        .then(|o| Ok::<_, std::convert::Infallible>(PathBuf::from(o.value())))?;
    let server_name: String = noargs::opt("server-name")
        .ty("NAME")
        .doc("TLS server name (SNI / certificate verification)")
        .default("localhost")
        .take(args)
        .then(|o| Ok::<_, std::convert::Infallible>(o.value().to_string()))?;
