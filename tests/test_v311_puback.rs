...
fn reserved_header_flags_are_rejected() {
    let puback = PubAck { packet_id: 1234 };
    let mut buf = [0u8; 16];
    let len = puback.encode(&mut buf).expect("エンコードに成功すること");
    // 固定ヘッダーの予約フラグ（bit 0-3）を設定する。
    buf[0] |= 0x0F;
    assert_eq!(
        PubAck::decode(&buf[..len]),
        Err(DecodeError::InvalidPacketFlags)
    );
}

#[test]
fn extra_bytes_after_puback_are_rejected() {
    let puback = PubAck { packet_id: 1234 };
    let mut buf = [0u8; 16];
    let len = puback.encode(&mut buf).expect("エンコードに成功すること");
    // 残り長さを 1 バイト増やす。
