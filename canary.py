...
# cargo update shiguredo_mqtt を実行
def run_cargo_update(dry_run: bool) -> None:
    if dry_run:
        print("Dry-run: Would run 'cargo update shiguredo_mqtt'")
    else:
        subprocess.run(["cargo", "update", "shiguredo_mqtt"], check=True)
        print("cargo update shiguredo_mqtt executed")
